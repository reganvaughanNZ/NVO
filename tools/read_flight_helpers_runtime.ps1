...
public static class NvoFlightHelpersReadOnly {
...
$nvoHandle = [NvoFlightHelpersReadOnly]::OpenProcess(0x1010, $false, [uint32]$nvoProcess.Id)
...
function Read-CodeBytes([UInt32]$Address, [int]$Count) {
    ...
    $got = [UIntPtr]::Zero
    if (-not [NvoFlightHelpersReadOnly]::ReadProcessMemory($nvoHandle, [IntPtr][long]$Address, $bytes, [UIntPtr][uint32]$Count, [ref]$got) -or $got.ToUInt64() -ne $Count) {
...
try {
    ...
    $nvoDest = [IO.Path]::GetFullPath((Join-Path $nvoWorkspace ('source\combat\step3c\runtime-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))))
    ...
    $null = [NvoFlightHelpersReadOnly]::CloseHandle($nvoHandle)
}
