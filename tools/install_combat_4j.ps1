...
$nvoStep = Join-Path $nvoRoot 'source\combat\step4j'
...
$nvoRelease = Join-Path $nvoRoot 'release\NVO-Combat-Packet-4J-Damage-Routes'
if ($nvoGame -ne 'C:\Users\regan\Desktop\Steam Install Folder\steamapps\common\Fallout New Vegas') { throw 'Unexpected game root.' }
if ($nvoPlan.packet -ne '4J' -or $nvoPlan.native_version -ne 330) { throw 'Unexpected packet.' }
...
$nvoExpected = @{
    'Data/NVSE/Plugins/NVOCombatCore.dll'='d2abf997cec4fec41ea613386938032178c062a66bc8c2e9849779216b1cdaa0'
    'Data/NVSE/Plugins/NVOCombatCore.pdb'='92b8cc0c12995c16fa14cec47f8dc73e775bf34e2a2291d524cd910a3fa0dd18'
    'NVOComponentKit4J.txt'='10ed24188f14802e5cd21fd26f7ca2070e10ad09bf0a465e797f898de0cc4e0a'
}
if (@($nvoPlan.files).Count -ne 3 -or @($nvoPlan.files.path | Select-Object -Unique).Count -ne 3) { throw 'Exactly three unique files required.' }
...
$nvoBackup = Child $nvoRoot ('backups\combat-install-4J-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8))
...
try {
    ...
    foreach ($row in $nvoPlan.files) {
        ...
        # CreateNew establishes that this transaction owns a newly created file.
        # Record it before streaming so a partial new kit is rolled back too.
        $outputStream = [IO.File]::Open($dest,[IO.FileMode]::CreateNew,[IO.FileAccess]::Write,[IO.FileShare]::None)
        if ($row.path -notin $nvoTouched) { $nvoTouched += $row.path }
        $inputStream = $null
        try {
            $inputStream = [IO.File]::OpenRead([string]$row.source)
