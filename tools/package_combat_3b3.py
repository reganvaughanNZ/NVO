"""Inspect and package packet 3B3 without loading its DLL or running the game."""
import argparse
import hashlib
import json
...
def inspect_pair(build):
    ...
    assert b'NVOCombatCore 0.3.3 | phase=3B3' in dll
...
def main():
    ...
    evidence['manual_review'] = [
        ...
        'CurrentHit, NativeObserver, DamageEvents and NativeLog source bytes unchanged from packet 302.',
        'Only code-pointer instrumentation is written in-process; no game object, projectile state or damage setter.',
    ]
    previous = ROOT / 'release/NVO-Combat-Packet-3B2-Compiled/Source/NVOCombatCore'
    for n in ('CurrentHit.cpp', 'NativeObserver.cpp', 'DamageEvents.cpp', 'NativeLog.cpp'):
    ...
    (PROJECT / 'BUILD-RESULT.md').write_text(f'''# Packet 3B3 build result

NVOCombatCore 0.3.3 / 303, Win32 MSVC 19.51.36257.0, /W4 /permissive- /O2 /MT /Zi. Build: out/{build.name}. Zero compiler warnings/errors. Expected two NVSE exports; matching DLL/PDB GUID and age. New timing entry/return assembly and unchanged copy wrapper inspected. No DLL execution, gameplay or GECK test.
...
This packet changes only DLL/PDB. The existing 3B2 test ESP, INI and activation are retained. Native flight/damage state writes remain disabled; one guarded missile dispatch slot supplies observation. Runtime acceptance awaits the user's six-shot/reload capture. Complete compile and object-disassembly evidence accompanies the release.
''', encoding='utf-8')
    ...
    manifest = {
        'packet': '3B3', 'version': '0.3.3', 'plugin_version': 303, 'prepared': '2026-09-15',
        ...
        'installation_state_note': 'Packaging state. Separate INSTALL-3B3 receipt records the transaction.',
        'new_dependencies': [], 'requires_existing_packet': '3B2 test ESP/INI and activation',
        'new_observer_vtable_slots': ['0108FD54'], 'new_native_event_handlers': 0,
        'maximum_timing_lifetimes_per_capture': 8, 'maximum_steps_per_lifetime': 64, 'maximum_nested_continuations_per_thread': 8,
    ...
    release = ROOT / 'release/NVO-Combat-Packet-3B3-Compiled'
    ...
    shutil.copyfile(ROOT / 'tools/install_combat_3b3.ps1', release / 'Installation/install_combat_3b3.ps1')
    with zipfile.ZipFile(ROOT / 'release/NVO-Combat-Packet-3B3-Source.zip', 'w', zipfile.ZIP_DEFLATED) as z:
        for p in source: z.write(p, 'native/NVOCombatCore/' + p.relative_to(PROJECT).as_posix())
        for n in ('package_combat_3b3.py', 'install_combat_3b3.ps1'):
            z.write(ROOT / 'tools' / n, 'tools/' + n)
        z.write(ROOT / 'source/combat/step3b3/HOOK-EVIDENCE.json', 'source/combat/step3b3/HOOK-EVIDENCE.json')
    target = ROOT / 'release/NVO-Combat-Packet-3B3-Compiled.zip'
