"""Inspect and package packet 3B1 without loading its DLL or running the game."""
import argparse
import hashlib
import json
...
def inspect_pair(build):
    ...
    assert b'NVOCombatCore 0.3.1 | phase=3B1' in dll
...
def main():
    ...
    evidence['manual_review'] = ['Existing CopyObserver assembly retains register/FP saves and one original tail jump.',
                               'FlightPreview object imports read-only Win32 file/metadata access, guarded reads and NVO logging.',
                               'No new event registration, setter, process-memory write or persistent game pointer in FlightPreview.',
...
Native flight-preview code and configuration are built/prepared; runtime profile resolution, measurements and reload behaviour await the user's capture. No flight/damage write authority exists in this packet. Full build log, DLL details and object disassemblies accompany the compiled delivery.
''', encoding='utf-8')
    ...
    assert ini.read_bytes() == (ROOT / 'source/combat/step3b1/NVOFlightPreview.ini').read_bytes()
    installed.append({'path': 'Data/NVSE/Plugins/NVOFlightPreview.ini', 'sha256': digest(ini)})
    manifest = {'packet': '3B1', 'version': '0.3.1', 'plugin_version': 301, 'prepared': '2026-09-15',
                ...
                'assistant_built': True, 'assistant_game_tested': False, 'new_dependencies': [],
                'engine_damage_calculation_hooks': False, 'flight_writes': False, 'damage_replacement_enabled': False,
                'new_native_event_handlers': 0, 'pilot_profiles': 2, 'maximum_profiles': 8, 'maximum_sampled_matches_per_capture': 32,
                ...
                'limits': 'Read-only settings and engine-counter samples, not a verified flight integrator or committed-damage measurement.'}
    (PROJECT / 'manifest.json').write_text(json.dumps(manifest, indent=2) + '\n', encoding='utf-8')
    source.append(PROJECT / 'manifest.json')
    release = ROOT / 'release/NVO-Combat-Packet-3B1-Compiled'
    ...
    with zipfile.ZipFile(ROOT / 'release/NVO-Combat-Packet-3B1-Source.zip', 'w', zipfile.ZIP_DEFLATED) as z:
        for p in source:
            z.write(p, 'NVOCombatCore/' + p.relative_to(PROJECT).as_posix())
    target = ROOT / 'release/NVO-Combat-Packet-3B1-Compiled.zip'
