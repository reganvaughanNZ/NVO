...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3g1\INSTALL-3G1-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3g1\INSTALL-3G1-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
if ((Get-FileHash -LiteralPath $nvoPlanPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne '02641493d1375d5f91f680548ae885c6d166fc6896f2c42a6d8aa67526e4ea22') { throw 'Prepared plan changed.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '3G1' -or $nvoPlan.version -ne 316 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3G1-Compiled'
...
foreach ($dependency in $nvoPlan.dependency_preconditions) {
    if ($dependency.path -notin @('Data/NVSE/Plugins/itr-nvse.dll', 'Data/NVSE/Plugins/jip_nvse.dll', 'Data/NVSE/Plugins/ShowOffNVSE.dll', 'Data/NVSE/Plugins/NVOCombatCore.dll', 'Data/NVSE/Plugins/NVOCombatCore.pdb', 'Data/NVSE/Plugins/NVOFlightPhysics.ini', 'Data/NVOFlightPilot.esp', 'Data/NVSE/Plugins/NVOFlightPreview.ini', 'NVOFlightKit3F.txt')) { throw 'Unexpected dependency precondition.' }
...
$nvoBackupName = 'backups\combat-install-3G1-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3G1'; version=316; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3g1\INSTALL-3G1-result.json') -Encoding UTF8
