...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3d\INSTALL-3D-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3d\INSTALL-3D-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '3D' -or $nvoPlan.version -ne 311 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3D-Compiled'
$nvoAllowedInstall = @('Data/NVSE/Plugins/NVOCombatCore.dll', 'Data/NVSE/Plugins/NVOCombatCore.pdb', 'Data/NVSE/Plugins/NVOFlightPreview.ini', 'Data/NVSE/Plugins/NVOFlightPhysics.ini', 'Data/NVOFlightPilot.esp', 'NVOFlightKit.txt')
if (@($nvoPlan.files).Count -ne 6 -or @($nvoPlan.files.path | Select-Object -Unique).Count -ne 6) { throw 'Exactly six unique NVO install files required.' }
foreach ($row in $nvoPlan.files) {
    ...
    } else { throw 'Only the six named NVO install actions are allowed.' }
...
$nvoBackupName = 'backups\combat-install-3D-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3D'; version=311; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3d\INSTALL-3D-result.json') -Encoding UTF8
