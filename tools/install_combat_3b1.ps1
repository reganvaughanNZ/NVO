...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3b1\INSTALL-3B1-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3B1-Compiled'
$nvoAllowedInstall = @('Data/NVSE/Plugins/NVOCombatCore.dll', 'Data/NVSE/Plugins/NVOCombatCore.pdb', 'Data/NVSE/Plugins/NVOFlightPreview.ini')
...
$nvoBackupName = 'backups\combat-install-3B1-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3B1'; version=301; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3b1\INSTALL-3B1-result.json') -Encoding UTF8
