...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3u\INSTALL-3U-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3u\INSTALL-3U-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
if ((Get-FileHash -LiteralPath $nvoPlanPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne '59e99ab8edeaff625de01cc96b36c959d79919085ea6b8ca97e3c8b55a176c2b') { throw 'Prepared plan changed.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '3U' -or $nvoPlan.version -ne 324 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3U-Contact-Speed'
...
$nvoBackupName = 'backups\combat-install-3U-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3U'; version=324; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3u\INSTALL-3U-result.json') -Encoding UTF8
