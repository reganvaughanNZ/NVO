...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3c4\INSTALL-3C4-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3C4-Compiled'
...
$nvoBackupName = 'backups\combat-install-3C4-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3C4'; version=309; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3c4\INSTALL-3C4-result.json') -Encoding UTF8
