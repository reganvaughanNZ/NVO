...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3f\INSTALL-3F-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3f\INSTALL-3F-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
if ((Get-FileHash -LiteralPath $nvoPlanPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne '1d83b20ae43e263038a23d66688a487bc4cd6d67ca4b2be9cf0421e3bd0116e3') { throw 'Prepared plan changed.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '3F' -or $nvoPlan.version -ne 311 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
if (@(Get-Process GECK,Vortex -ErrorAction SilentlyContinue).Count) { throw 'Close GECK and Vortex before installing this packet.' }

# Validate the exact allowlist and release bytes before touching the running game.
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3F-Update'
$nvoAllowedInstall = @('Data/NVSE/Plugins/NVOFlightPreview.ini', 'Data/NVOFlightPilot.esp', 'NVOFlightKit3F.txt')
if (@($nvoPlan.files).Count -ne 3 -or @($nvoPlan.files.path | Select-Object -Unique).Count -ne 3) { throw 'Exactly three unique NVO install files required.' }
foreach ($row in $nvoPlan.files) {
    ...
    } else { throw 'Only the three named NVO install actions are allowed.' }
...
foreach ($dependency in $nvoPlan.dependency_preconditions) {
    if ($dependency.path -notin @('Data/NVSE/Plugins/itr-nvse.dll', 'Data/NVSE/Plugins/jip_nvse.dll', 'Data/NVSE/Plugins/ShowOffNVSE.dll', 'Data/NVSE/Plugins/NVOCombatCore.dll', 'Data/NVSE/Plugins/NVOCombatCore.pdb', 'Data/NVSE/Plugins/NVOFlightPhysics.ini')) { throw 'Unexpected dependency precondition.' }
...
$nvoBackupName = 'backups\combat-install-3F-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3F'; version=311; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3f\INSTALL-3F-result.json') -Encoding UTF8
