...
public static class NvoFlightDirectionReadOnly {
...
$nvoHandle = [NvoFlightDirectionReadOnly]::OpenProcess(0x1010, $false, [uint32]$nvoProcess.Id)
...
function Read-CodeBytes([UInt32]$Address, [int]$Count) {
    ...
    $got = [UIntPtr]::Zero
    if (-not [NvoFlightDirectionReadOnly]::ReadProcessMemory($nvoHandle, [IntPtr][long]$Address, $bytes, [UIntPtr][uint32]$Count, [ref]$got) -or $got.ToUInt64() -ne $Count) {
...
try {
    ...
    $nvoRanges = @(
        @{name='sincos'; address=0x4169A0; count=0x80},
        @{name='projectile_virtual'; address=0x8D0360; count=0x40},
        @{name='sit_state'; address=0xACBB70; count=0x40},
        @{name='has_health'; address=0x5722C0; count=0x100},
        @{name='mobile_process'; address=0x9306D0; count=0x100},
        @{name='pitch_getter'; address=0x931D70; count=0x100},
        @{name='heading_getter'; address=0x80F790; count=0x200},
        @{name='rotation_getter'; address=0x430830; count=0x40},
        @{name='pitch_matrix'; address=0x524AC0; count=0x100},
        @{name='vector_add'; address=0x63C8A0; count=0x60},
        @{name='generic_caller'; address=0x8A6300; count=0x100},
        @{name='missile_vtable'; address=0x108FA44; count=0x400},
        @{name='speed'; address=0x9669C0; count=0x200},
        @{name='vector_constructor'; address=0x416870; count=0x100},
        @{name='movement'; address=0x92F260; count=0x1420},
        @{name='matrix_vector'; address=0x4B4500; count=0xB0},
        @{name='rotation_matrix'; address=0x4A0C90; count=0x300},
        @{name='timescale'; address=0x716440; count=0x100},
        @{name='projectile_update'; address=0x9B8030; count=0x870},
    ...
    $nvoDest = [IO.Path]::GetFullPath((Join-Path $nvoWorkspace ('source\combat\step3c2\runtime-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))))
    ...
    $null = [NvoFlightDirectionReadOnly]::CloseHandle($nvoHandle)
}
