...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3e\INSTALL-3E-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3e\INSTALL-3E-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '3E' -or $nvoPlan.version -ne 311 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3E-Update'
$nvoAllowedInstall = @('Data/NVSE/Plugins/NVOFlightPreview.ini', 'Data/NVOFlightPilot.esp', 'NVOFlightKit3E.txt')
...
$nvoBackupName = 'backups\combat-install-3E-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3E'; version=311; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3e\INSTALL-3E-result.json') -Encoding UTF8
