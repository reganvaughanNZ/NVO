...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3b3a\INSTALL-3B3A-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3B3A-Compiled'
...
$nvoBackupName = 'backups\combat-install-3B3A-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3B3A'; version=304; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3b3a\INSTALL-3B3A-result.json') -Encoding UTF8
