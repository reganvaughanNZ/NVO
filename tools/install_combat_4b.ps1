...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step4b\INSTALL-4B-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step4b\INSTALL-4B-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
if ((Get-FileHash -LiteralPath $nvoPlanPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne '59a2eddb07158667be5ffc771e6935519c19b2ed7261314c9d9692c4fa6370d2') { throw 'Prepared plan changed.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '4B' -or $nvoPlan.version -ne 326 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
if (@(Get-Process GECK,Vortex -ErrorAction SilentlyContinue).Count) { throw 'Close GECK and Vortex before installation.' }

# Validate the exact allowlist and release bytes before touching the running game.
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-4B-Guarded-Armour-Snapshot'
$nvoAllowedInstall = @('Data/NVSE/Plugins/NVOCombatCore.dll', 'Data/NVSE/Plugins/NVOCombatCore.pdb', 'NVOArmourKit4B.txt', 'NVOArmourTarget4B.txt')
if (@($nvoPlan.files).Count -ne 4 -or @($nvoPlan.files.path | Select-Object -Unique).Count -ne 4) { throw 'Exactly four unique NVO install files required.' }
foreach ($row in $nvoPlan.files) {
    ...
    } else { throw 'Only the four named NVO install actions are allowed.' }
    if ($row.path -match '\.(esm|bsa|fos)$') { throw 'ESMs/assets/saves must not be changed.' }
}

foreach ($dependency in $nvoPlan.dependency_preconditions) {
    if ($dependency.path -notin @('Data/NVSE/Plugins/itr-nvse.dll', 'Data/NVSE/Plugins/jip_nvse.dll', 'Data/NVSE/Plugins/ShowOffNVSE.dll', 'Data/NVSE/Plugins/NVOCombatCore.dll', 'Data/NVSE/Plugins/NVOCombatCore.pdb', 'Data/NVSE/Plugins/NVOFlightPhysics.ini', 'Data/NVOFlightPilot.esp', 'Data/NVSE/Plugins/NVOFlightPreview.ini', 'NVOFlightKit3F.txt', 'Data/NVO.esm')) { throw 'Unexpected dependency precondition.' }
    $dependencyPath = Assert-ChildPath $nvoGame $dependency.path
    if ((File-Hash $dependencyPath) -ne $dependency.sha256) { throw 'An inspected dependency or foundation file changed. Installation stopped without modifying game files.' }
}

if (Test-Path -LiteralPath (Join-Path $nvoGame 'Data\RD.esm')) { throw 'RD.esm must remain absent.' }
...
$nvoBackupName = 'backups\combat-install-4B-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='4B'; version=326; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step4b\INSTALL-4B-result.json') -Encoding UTF8
