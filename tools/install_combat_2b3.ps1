...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step2\INSTALL-2B3-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-2B3-Compiled'
...
foreach ($row in $nvoPlan.files) {
    ...
    } elseif ($row.action -ne 'archive_remove') { throw 'Unsupported action.' }
...
$nvoBackupName = 'backups\combat-install-2B3-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='2B3'; version=202; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step2\INSTALL-2B3-result.json') -Encoding UTF8
