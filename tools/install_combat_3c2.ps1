...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3c2\INSTALL-3C2-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3C2-Compiled'
...
$nvoBackupName = 'backups\combat-install-3C2-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3C2'; version=307; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3c2\INSTALL-3C2-result.json') -Encoding UTF8
