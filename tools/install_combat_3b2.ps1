...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3b2\INSTALL-3B2-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3B2-Compiled'
$nvoAllowedInstall = @('Data/NVSE/Plugins/NVOCombatCore.dll', 'Data/NVSE/Plugins/NVOCombatCore.pdb', 'Data/NVSE/Plugins/NVOFlightPreview.ini', 'Data/NVOFlightPilot.esp', 'NVOFlightPilot.txt')
if (@($nvoPlan.files).Count -ne 5 -or @($nvoPlan.files.path | Select-Object -Unique).Count -ne 5) { throw 'Exactly five unique NVO install files required.' }
foreach ($row in $nvoPlan.files) {
    ...
    } else { throw 'Only the five named NVO install actions are allowed.' }
    if ($row.path -match '\.(esm|bsa|fos)$' -or ($row.path -match '\.esp$' -and $row.path -ne 'Data/NVOFlightPilot.esp')) { throw 'Game plugins/assets/saves must not be changed.' }
}
...
$nvoActivationRoot = 'C:\Users\regan\AppData\Local\FalloutNV'
$nvoActivation = Assert-ChildPath $nvoActivationRoot 'plugins.txt'
$nvoActivationSource = Assert-ChildPath $nvoWorkspace 'source\combat\step3b2\plugins.after.txt'
if ($nvoActivation -ne $nvoPlan.activation.path -or $nvoActivationSource -ne $nvoPlan.activation.source) { throw 'Unexpected activation file.' }
if ((File-Hash $nvoActivation) -ne $nvoPlan.activation.before_sha256 -or (File-Hash $nvoActivationSource) -ne $nvoPlan.activation.source_sha256) { throw 'Activation state changed since preparation.' }
$nvoOriginalActivationBytes = [IO.File]::ReadAllBytes($nvoActivation)
$nvoAppendBytes = [Text.Encoding]::ASCII.GetBytes("NVOFlightPilot.esp`r`n")
$nvoExpectedActivationBytes = [byte[]]($nvoOriginalActivationBytes + $nvoAppendBytes)
if ([Convert]::ToBase64String($nvoExpectedActivationBytes) -ne [Convert]::ToBase64String([IO.File]::ReadAllBytes($nvoActivationSource))) { throw 'Activation source must only append the pilot entry.' }
if ([Text.Encoding]::UTF8.GetString($nvoOriginalActivationBytes) -match '(?im)^\*?NVOFlightPilot\.esp\s*$') { throw 'Pilot already active; review plan.' }
...
$nvoBackupName = 'backups\combat-install-3B2-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoActivationBackup = Assert-ChildPath $nvoBackup 'originals\activation\plugins.txt'
$null = New-Item -ItemType Directory -Path ([IO.Path]::GetDirectoryName($nvoActivationBackup)) -Force
if ((File-Hash $nvoActivation) -ne $nvoPlan.activation.before_sha256) { throw 'Activation changed before backup.' }
Copy-Item -LiteralPath $nvoActivation -Destination $nvoActivationBackup
if ((File-Hash $nvoActivationBackup) -ne $nvoPlan.activation.before_sha256) { throw 'Activation backup mismatch.' }
$nvoLoadOrder = Assert-ChildPath $nvoActivationRoot 'loadorder.txt'
if (Test-Path -LiteralPath $nvoLoadOrder -PathType Leaf) { $nvoProtected += [PSCustomObject]@{ path=$nvoLoadOrder; sha256=(File-Hash $nvoLoadOrder) } }
$nvoBefore = @()
foreach ($row in $nvoPlan.files) {
...
try {
    ...
    if ((File-Hash $nvoActivation) -ne $nvoPlan.activation.before_sha256) { throw 'Activation changed before enabling pilot.' }
    $nvoActivationTouched = $true
    Remove-Item -LiteralPath $nvoActivation -Force
    Copy-Item -LiteralPath $nvoActivationSource -Destination $nvoActivation
    if ((File-Hash $nvoActivation) -ne $nvoPlan.activation.source_sha256) { throw 'Activation update mismatch.' }
    ...
    $rollbackErrors = @()
    if ($nvoActivationTouched) {
        try {
            if (Test-Path -LiteralPath $nvoActivation -PathType Leaf) { Remove-Item -LiteralPath $nvoActivation -Force }
            Copy-Item -LiteralPath $nvoActivationBackup -Destination $nvoActivation
            if ((File-Hash $nvoActivation) -ne $nvoPlan.activation.before_sha256) { throw 'Activation restoration mismatch.' }
        } catch { $rollbackErrors += "plugins.txt : $($_.Exception.Message)" }
    }
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3B2'; version=302; backup=$nvoBackup;
    closed_processes=$nvoClosed;
    activation=@{path=$nvoActivation; sha256=(File-Hash $nvoActivation); appended='NVOFlightPilot.esp'; original_entries_preserved=$true};
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3b2\INSTALL-3B2-result.json') -Encoding UTF8
