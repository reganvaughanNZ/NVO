...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step2\INSTALL-2B4-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-2B4-Compiled'
...
foreach ($row in $nvoPlan.files) {
    ...
    } else { throw 'Only the two NVO DLL/PDB install actions are allowed.' }
...
foreach ($dependency in $nvoPlan.dependency_preconditions) {
    if ($dependency.path -ne 'Data/NVSE/Plugins/itr-nvse.dll') { throw 'Unexpected dependency precondition.' }
    $dependencyPath = Assert-ChildPath $nvoGame $dependency.path
    if ((File-Hash $dependencyPath) -ne $dependency.sha256) { throw 'ITR binary changed since ABI review. Installation stopped without modifying game files.' }
}
...
$nvoBackupName = 'backups\combat-install-2B4-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='2B4'; version=203; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step2\INSTALL-2B4-result.json') -Encoding UTF8
