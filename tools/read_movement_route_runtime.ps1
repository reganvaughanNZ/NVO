...
public static class NvoMovementRouteReadOnly {
...
$nvoHandle = [NvoMovementRouteReadOnly]::OpenProcess(0x1010, $false, [uint32]$nvoProcess.Id)
...
function Read-CodeBytes([UInt32]$Address, [int]$Count) {
    ...
    $got = [UIntPtr]::Zero
    if (-not [NvoMovementRouteReadOnly]::ReadProcessMemory($nvoHandle, [IntPtr][long]$Address, $bytes, [UIntPtr][uint32]$Count, [ref]$got) -or $got.ToUInt64() -ne $Count) {
...
try {
    ...
    $nvoRanges = @(
        @{name='position_commit'; address=0x575830; count=0x500},
        @{name='controller_move'; address=0xC70B60; count=0x100},
        @{name='node_getter'; address=0x43FCD0; count=0x100},
        @{name='movement'; address=0x92F260; count=0x1420},
        @{name='projectile_move'; address=0x9BF300; count=0x170},
        @{name='movement_thread'; address=0x9304A0; count=0x300},
        @{name='controller_mode'; address=0x621240; count=0x100},
        @{name='vector_transform'; address=0x43C490; count=0x100},
        @{name='transform_point'; address=0x439E90; count=0x100},
        @{name='is_actor'; address=0x47C850; count=0x40},
    ...
    $nvoDest = [IO.Path]::GetFullPath((Join-Path $nvoWorkspace ('source\combat\step3c3\runtime-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))))
    ...
    $null = [NvoMovementRouteReadOnly]::CloseHandle($nvoHandle)
}
