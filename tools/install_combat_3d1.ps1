...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3d1\INSTALL-3D1-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3d1\INSTALL-3D1-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '3D1' -or $nvoPlan.version -ne 311 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3D1-Update'
$nvoAllowedInstall = @('Data/NVSE/Plugins/NVOFlightPreview.ini', 'Data/NVOFlightPilot.esp')
...
$nvoBackupName = 'backups\combat-install-3D1-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3D1'; version=311; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3d1\INSTALL-3D1-result.json') -Encoding UTF8
