...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3b3\INSTALL-3B3-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3B3-Compiled'
...
$nvoBackupName = 'backups\combat-install-3B3-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3B3'; version=303; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3b3\INSTALL-3B3-result.json') -Encoding UTF8
