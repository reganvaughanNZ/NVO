"""Inspect and package native324; prepare an exact two-file install transaction."""
...
CORE=ROOT/'native/NVOCombatCore'; OUT=ROOT/'source/combat/step3u'
RELEASE=ROOT/'release/NVO-Combat-Packet-3U-Contact-Speed'
BUILD=CORE/'out/build-24180-5912'
...
def main():
 assert not (OUT/'INSTALL-3U-result.json').exists()
 report=json.loads((OUT/'REPLAY-RESULT.json').read_text())
 for path,h in report['source_sha256'].items():assert sha(ROOT/path)==h,path
 scope=vars(pair).copy()
 exec(inspect.getsource(pair.inspect_pair).replace('0.3.4 | phase=3B3A','0.3.24 | phase=3U'),scope)
 ...
 assert b'IMPACT_SPEED ' in data and b'IMPACT_TERRAIN ' in data
 prior=ROOT/'release/NVO-Combat-Packet-3R1-Normal-Build/Source/NVOCombatCore'
 snap=json.loads((ROOT/'source/combat/step3r1/SOURCE-SNAPSHOT.json').read_text())['source_sha256']
 changes={'src/FlightImpact.inl','src/FlightImpactJoin.inl','src/FlightPhysics.cpp','src/Plugin.cpp','CMakeLists.txt'}
 unchanged=[];diff=[]
 ...
 for name in ['src/FlightContactSpeed.inl','tests/contact_speed_checks.inl']:
  diff.append('\nNEW: '+name+'\n'+(CORE/name).read_text())
 ...
 asm=[]
 for build,label in [(CORE/'out/build-11026-30627','native323'),(BUILD,'native324')]:
 ...
 evidence.update(packet='3U',native_version=324,unchanged_inputs=unchanged,unchanged_naked_bridges=bridges,
 ...
 manifest=dict(packet='3U',version='0.3.24',plugin_version=324,status='Compiled; installation and user runtime checkpoint pending',
 ...
 buildtext=f'''# Native324 / Packet3U build

Win32 MSVC /W4 /O2 /MT /Zi; zero warnings/errors. Offline replay /W4 /WX passed. Matching DLL/PDB; exactly two NVSE exports. All {len(bridges)} physics bridge instruction bodies unchanged. {len(unchanged)} existing compilation/configuration inputs unchanged. New code is bounded diagnostic segment-speed evidence and read-only contact terrain sampling; damage off. Live acceptance pending.

DLL {evidence['dll_sha256']}
PDB {evidence['pdb_sha256']}
'''
 (CORE/'BUILD-RESULT.md').write_text(buildtext);(CORE/'PROGRESS.md').write_text('Packet3U: see README.md and source/combat/step3u. Runtime checkpoint pending. Ultra317 HOLD.\n')
 ...
 for name in ['STATIC-CHECKS.json','REPLAY-RESULT.json','REPLAY-ROWS.json','CAPTURE-EVIDENCE.json','SOURCE-DIFF.txt','ENGINE-EVIDENCE.json']:copy(OUT/name,RELEASE/'Evidence'/name)
 for name in ['build.log','dll-details.txt','toolchain.log']:copy(BUILD/name,RELEASE/'build-evidence'/name)
 for name in ['build.log','results.jsonl']:copy(OUT/'out'/name,RELEASE/'build-evidence'/('offline-'+name))
 for name in ['replay_contact_speed_3u.py','package_combat_3u.py','package_combat_3b3a.py','package_combat_3h.py']:copy(ROOT/'tools'/name,RELEASE/'Source/tools'/name)
 plan=json.loads((ROOT/'source/combat/step3r1/INSTALL-3R1-plan.json').read_text())
 plan.update(packet='3U',version=324,files=[])
 for ext in ['dll','pdb']:
 ...
 expected_pair={'dll':'a78d72dd22b7d605b150aa617ecb88b0047bde73c25f5f41b5cfea5b2d148d63','pdb':'e257619b26472cc46e23cc29803c16d608bbb1ed7d757449e8e5e67f0c31d1ee'}
 ...
 save(OUT/'INSTALL-3U-plan.json',plan);pin=sha(OUT/'INSTALL-3U-plan.json')
 installer=(ROOT/'tools/install_combat_3r1.ps1').read_text().replace('3R1','3U').replace('step3r1','step3u').replace('Normal-Build','Contact-Speed').replace('version -ne 323','version -ne 324').replace('version=323','version=324')
 installer=re.sub(r"-ne '[0-9a-f]{64}'","-ne '"+pin+"'",installer,count=1)
 (ROOT/'tools/install_combat_3u.ps1').write_text(installer)
 copy(ROOT/'tools/install_combat_3u.ps1',RELEASE/'Source/tools/install_combat_3u.ps1')
 copy(OUT/'INSTALL-3U-plan.json',RELEASE/'Installation/INSTALL-3U-plan.json')
 ...
 snapshot=dict(packet='3U',native_version=324,capacity_probe_enabled=False,source_sha256={str(p.relative_to(CORE)).replace('\\','/'):sha(p) for p in CORE.rglob('*') if p.is_file() and 'out' not in p.relative_to(CORE).parts})
 save(OUT/'SOURCE-SNAPSHOT.json',snapshot)
 html='''<!doctype html><html lang="en"><meta charset="utf-8"><title>NVO 3U — contact speed</title>
...
<h1>NVO 0.3.24 · Contact-speed coverage</h1><p>Close-range shots gain an observed segment-speed estimate. Off-curve contacts retain a bounded step-speed range. A new read-only observation checks post-collision terrain corrections.</p>
...
<h2>One short check</h2><p>Use standard ammunition and the existing weapons or <code>bat NVOFlightKit3F</code>. Keep NVO.esm and NVOFlightPilot.esp active.</p>
<ol><li>One close-range 9mm pistol hit on a living target, aimed normally.</li><li>One hunting-rifle shot into distant sloping ground.</li><li>One hunting-rifle VATS torso shot on a living target. Note a miss if it misses.</li><li>Reload once, then fire one 9mm shot into a nearby wall. Wait a few seconds and quit normally.</li></ol>
...
<p>These are conditional speed estimates, not exact contact timestamps. Pre-movement contacts without a checked vector remain unavailable.</p></html>'''
