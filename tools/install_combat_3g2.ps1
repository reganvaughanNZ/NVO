...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3g2\INSTALL-3G2-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3g2\INSTALL-3G2-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
if ((Get-FileHash -LiteralPath $nvoPlanPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne '0bc408513f6a3b9fae9392c25c48f487db71217b2661c7aa647a30474c0b3556') { throw 'Prepared plan changed.' }
$nvoPlan = Get-Content -LiteralPath $nvoPlanPath -Raw | ConvertFrom-Json
if ($nvoPlan.packet -ne '3G2' -or $nvoPlan.version -ne 317 -or @($nvoPlan.empty_directories).Count) { throw 'Unexpected packet or cleanup scope.' }
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3G2-Compiled'
...
$nvoBackupName = 'backups\combat-install-3G2-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3G2'; version=317; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3g2\INSTALL-3G2-result.json') -Encoding UTF8
