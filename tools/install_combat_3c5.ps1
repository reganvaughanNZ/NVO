...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3c5\INSTALL-3C5-plan.json'
if (Test-Path -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3c5\INSTALL-3C5-result.json')) { throw 'Installation already recorded; do not overwrite the transaction.' }
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3C5-Compiled'
...
$nvoBackupName = 'backups\combat-install-3C5-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3C5'; version=310; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3c5\INSTALL-3C5-result.json') -Encoding UTF8
