...
$nvoPlanPath = Join-Path $nvoWorkspace 'source\combat\step3c3\INSTALL-3C3-plan.json'
...
$nvoRelease = Join-Path $nvoWorkspace 'release\NVO-Combat-Packet-3C3-Compiled'
...
$nvoBackupName = 'backups\combat-install-3C3-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8)
...
$nvoResult = [PSCustomObject]@{
    status='installed'; packet='3C3'; version=308; backup=$nvoBackup;
...
$nvoResult | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $nvoWorkspace 'source\combat\step3c3\INSTALL-3C3-result.json') -Encoding UTF8
