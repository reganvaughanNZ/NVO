...
$nvoStep = Join-Path $nvoRoot 'source\combat\step4j1'
...
$nvoRelease = Join-Path $nvoRoot 'release\NVO-Combat-Packet-4J1-Flame-Lifecycle'
if ($nvoGame -ne 'C:\Users\regan\Desktop\Steam Install Folder\steamapps\common\Fallout New Vegas') { throw 'Unexpected game root.' }
if ($nvoPlan.packet -ne '4J1' -or $nvoPlan.native_version -ne 331) { throw 'Unexpected packet.' }
...
$nvoExpected = @{
    'Data/NVSE/Plugins/NVOCombatCore.dll'='5eb95e8acdfda671650090b07d4a0e84ceb0a3c7785efae4c06f4cdadad77e86'
    'Data/NVSE/Plugins/NVOCombatCore.pdb'='a838f1636bb6f70add7717c30d0a6e763b54410970ca37f849c1da6c9a8afaa3'
}
...
$nvoBackup = Child $nvoRoot ('backups\combat-install-4J1-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-' + [guid]::NewGuid().ToString('N').Substring(0,8))
...
try {
    ...
    foreach ($row in $nvoPlan.files) {
        ...
        # Record it before streaming so a partial new file is rolled back too.
