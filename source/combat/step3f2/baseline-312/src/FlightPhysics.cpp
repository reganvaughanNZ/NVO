...
void nvo::physics::Initialize() noexcept
{
    ...
    // Eight spans occupy four distinct pages. The generic span ends exactly at
    ...
    Page pages[]={{kMoveSite,static_cast<unsigned>(kSite+5-kMoveSite)}, {kCommitSite,5},
...
void nvo::physics::BeginCapture(unsigned session) noexcept
{
    ...
    // NVO_3F1_DIAGNOSTICS_BEGIN
    nvo::log::Write("PHYSICS_DIAGNOSTIC_READY session=%u schema=1 report_limit=%u rows_per_report=4 failure_rows_use_priority_reserve=1 routine_lifetimes=%u routine_entries=%u no_extra_engine_reads=1 physics_rules_unchanged=1",
