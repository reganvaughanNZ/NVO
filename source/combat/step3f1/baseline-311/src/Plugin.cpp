...
constexpr std::uint32_t kPluginVersion = 311; // 0.3.11, packet 3D per-shot four-weapon profiles
    ...
    nvo::nvse::PluginInfo* info) noexcept
{
    ...
    nvo::log::Write("NVOCombatCore 0.3.11 | phase=3D | native_events=1 | current_hit_observer_pending=1 | itr_damage_events_pending=1 | flight_preview=1 | flight_timing_pending=1 | native_physics_pending=1 | per_shot_selection=1 | movement_argument_pilot=1 | isolated_record_flight_pilot=1 | hit_hooks=0 | damage_replacement=0");
