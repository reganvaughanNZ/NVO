...
enum class ImpactTargetKind : U32 { Invalid, World, Reference };
ImpactTargetKind ClassifyImpactTarget(const void* target,U32 formId) noexcept
{
    if (!target) return formId?ImpactTargetKind::Invalid:ImpactTargetKind::World;
    return formId?ImpactTargetKind::Reference:ImpactTargetKind::Invalid;
}
struct ContactTerrain {
    unsigned samples{};
    bool valid{}, queryOk{};
    U32 target{},flags{};
...
struct Track {
    ...
    ContactTerrain contactTerrain{};
};
Track gTracks[kCapacity]{};
nvo::admission::Pool<kCapacity> gSlots;
unsigned gReservations{},gCancellations{};
...
{ if (!p) return nullptr; for (auto& t : gTracks) if (t.serial && t.p == p && (!serial || t.serial == serial)) return &t; return nullptr; }
...
void Reject(Track& t, const char* reason) noexcept
{
    ...
    nvo::admission::Block(nvo::admission::Fault::Physics);
    gSlots.Fault(t.ticket);
    if (gRejected<=16) nvo::log::Write("PHYSICS_REJECT session=%u lifetime=%llu reason=%s process_admissions_blocked=1 further_NVO_updates_for_this_projectile=0 prior_changes_rolled_back=0 private_projectile_left_to_engine=1",gSession,t.serial,reason);
}
// NVO_3F1_DIAGNOSTICS_BEGIN
...
void __cdecl BeforeMovement(void* owner,float adjustedDt,float* input,std::uintptr_t frame) noexcept
{
    ...
    ++t->movementEntries;
    if (t->stopped) return;
    if (!t->committed) { Reject(*t,"movement_before_creation_return"); return; }
    ...
    const float* candidate,float* height,U32 result) noexcept
{
    ...
    // Capture the collision BEFORE the engine's optional Z-only terrain clamp.
    // This is evidence for the contact-speed observer, never a movement repair.
    // Keep baseline contacts too, without changing the old write eligibility.
    auto& terrain=t->contactTerrain;
    void* contactHead{};
    if (Read(owner,0x88,contactHead) && contactHead && ImpactFind(t->serial)) {
        ++terrain.samples; terrain.valid=false;
        const auto contact=ReadImpact(*t);
        float observedFloor{}; Vec observedPosition{};
        if (terrain.samples==1 && contact.valid==15 && contact.policyRead && !contact.more
            && ImpactVector(candidate,0,observedPosition) && Read(height,0,observedFloor)
            && std::isfinite(observedFloor) && std::abs(observedFloor)<=1e7f) {
            terrain.valid=true; terrain.queryOk=(result&0xFFu)!=0;
            terrain.target=contact.target; terrain.targetKind=contact.targetKind;
            terrain.flags=contact.flags; terrain.region=contact.region;
            terrain.point=contact.point; terrain.position=observedPosition; terrain.floor=observedFloor;
        }
    }
    if (!t->wrote || !t->baselineVerified) return;
    ++gTerrainQueries;
...
bool nvo::physics::Attach(admission::Ticket ticket,void* p,U32 ref) noexcept
{
    const ErrorGuard error; const Lock lock;
    if (!gActive.load() || !p || !ref || gSlots.State(ticket)!=admission::Phase::Reserved || Find(p)) return false;
    if (!gSlots.Bind(ticket)) return false;
    auto& t=gTracks[ticket.slot]; t.p=p; t.ref=ref;
    return true;
}
bool nvo::physics::Commit(admission::Ticket ticket) noexcept
{
    const ErrorGuard error; const Lock lock;
    if (!gActive.load() || gSlots.State(ticket)!=admission::Phase::Bound) return false;
    auto& t=gTracks[ticket.slot];
    if (!t.p || t.stopped || t.committed || !gSlots.Commit(ticket)) return false;
    t.committed=true; t.logged=gAccepted++<kLoggedLives;
    ImpactEnroll(t);
    if (t.logged) nvo::log::Write("PHYSICS_TRACK session=%u lifetime=%llu projectile=%08X weapon=%08X ammo=%08X base=%08X profile=%u drag_model=G%u bc=%.9g admission=exact_return",gSession,t.serial,t.ref,t.weapon,t.ammo,t.base,t.profile,t.dragModel,t.ballisticCoefficient);
    return true;
}
bool nvo::physics::Cancel(admission::Ticket ticket) noexcept
{
    const ErrorGuard error; const Lock lock;
    if (!gSlots.Cancel(ticket)) return false;
    gTracks[ticket.slot]={}; ++gCancellations; return true;
}
void nvo::physics::Fault(admission::Ticket ticket) noexcept
{
    const ErrorGuard error; const Lock lock;
