...
void nvo::flight::BeginCapture(unsigned session, bool layoutReady, bool selectorReady) noexcept
{
    ...
    nvo::log::Write("FLIGHT_SELECTOR_READY session=%u ready=%u boundary_ready=%u thread=%u method=scoped_after_ShowOff_selection shared_record_writes=0 stock_record_overrides=0 damage_replacement=0",
...
bool nvo::flight::PrepareProjectile(void* actor,void* projectileBase,void* weapon,admission::Profile& output) noexcept
{
    const ErrorGuard guard;
    output={};
    if (!gActive || !gSelectorReady || admission::Blocked()) return false;
    if (GetCurrentThreadId()!=gCaptureThread) { ++gSelectionOtherThread; return false; }
    nvo::hit::Form w{},a{},b{};
    if (!nvo::hit::ReadForm(weapon,w) || w.type!=0x28) return false;
    bool known=false;
    for (unsigned i=0;i<gProfileCount;++i)
        if (gProfiles[i].flightEnabled && gProfiles[i].weapon.resolved==w.id) known=true;
    ...
    if (!nvo::hit::ReadForm(actor,a) || (a.type!=0x3B && a.type!=0x3C)
        ...
        SelectionSkip("equipped_ammo_unavailable",w.id,ammo); return false;
    }
    for (unsigned i=0;i<gProfileCount;++i) {
        auto& p=gProfiles[i];
        if (!p.flightEnabled || w.id!=p.weapon.resolved || ammo!=p.ammo.resolved) continue;
        const auto expected=p.selectOnFire?p.original.resolved:p.projectile.resolved;
        void* expectedPointer=p.selectOnFire?p.originalPointer:p.result.form;
        if (b.id!=expected || projectileBase!=expectedPointer) continue;
        ...
        if (!nvo::physics::Ready(gUnitsPerMetre) || !nvo::hit::ReadForm(p.result.form,r)
            ...
            || !ReadAt(p.result.form,0x60,replacement)
            || std::memcmp(current,p.selectOnFire?p.originalData:p.replacementData,84)
            || std::memcmp(replacement,p.replacementData,84)) {
            SelectionSkip("profile_changed_or_physics_unavailable",w.id,ammo); return false;
        }
        output={actor,weapon,p.result.form,a.id,w.id,ammo,r.id,b.id,i,p.dragModel,p.bc,p.result.form!=projectileBase};
        return true;
    }
    SelectionSkip("ammo_or_projectile_not_profiled",w.id,ammo);
    return false;
}
void nvo::flight::Selected(const admission::Profile& p,unsigned long long serial) noexcept
{
    const ErrorGuard guard;
    ++gSelected;
    if (gSelected<=32) nvo::log::Write("FLIGHT_SELECT session=%u selection=%u lifetime=%llu profile=%s source=%08X weapon=%08X ammo=%08X original=%08X replacement=%08X changed_base=%u reservation_before_selection=1 result_consumed=pending_return shared_record_writes=0",
        gSession,gSelected,serial,gProfiles[p.index].name,p.source,p.weapon,p.ammo,p.original,p.base,p.changesBase?1u:0u);
}
bool nvo::flight::ValidateCreation(void* projectile,const admission::Profile& p,U32 source,U32 weapon,U32 ammo) noexcept
{
    const ErrorGuard guard;
    Snapshot s{}; void *a{},*w{},*b{};
    return gActive && source==p.source && weapon==p.weapon && ammo==p.ammo
        && SnapshotOf(projectile,s) && s.ref && s.source==source && s.weapon==weapon && s.base==p.base
        && ReadAt(projectile,0xFC,a) && a==p.actor && ReadAt(projectile,0xF8,w) && w==p.weaponPointer
        && ReadAt(projectile,0x20,b) && b==p.projectile;
}
...
void nvo::flight::Create(void* projectile, U32 source, U32 weapon, U32 ammo, unsigned long long lifetime, bool tracked, bool admitted) noexcept
{
    const ErrorGuard guard;
    if (!gActive) return;
    ++gSeen;
    ...
    *slot = {lifetime, profile, state, false};
    ...
    if (admitted && p.flightEnabled && nvo::physics::Ready(gUnitsPerMetre))
