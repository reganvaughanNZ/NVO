# Current packet: 4B / 326 guarded equipped-armour snapshot

Step 4B adds no hook. It calls a read-only armour observer from the existing JIP `CopyHitData` C++ bridge after `HitTransaction::CopyInput` returns a valid exact scope and before the existing current-hit logger. The naked wrapper, JIP slots, ITR sites, flight sites, and every installed patch operation remain unchanged.

Only form type `0x3B` (`Character`) is eligible for an armour snapshot. Form type `0x3C` (`Creature`) and all other target types are summary-counted and skipped before consuming the 64 supported-snapshot budget. They are never represented as bare. Natural hide, shell, chassis, and baked/toggled BIP geometry require later physiology or inherent-protection profiles.

The reader manually and defensively traverses `TESObjectREFR::extraDataList`, exactly one `ExtraContainerChanges`, its owner-bound object list, entry extend lists, and exact instance extra lists. It recognizes `ExtraWorn`, rejects `ExtraWornLeft` armour, reads optional `ExtraHealth`, and ignores a verified equipped weapon. Unknown worn form types reject the complete snapshot. Fixed open-addressed pointer sets enforce cycle and cross-list alias rejection with a 64-probe limit. Hard bounds are 64 actor extras, 512 entry nodes, 1,024 instance nodes, 64 extras per instance, 8,192 total instance extras, and 32 output armour rows.

Every engine access goes through the existing SEH-protected `ReadBytes`; no engine helper, allocator, getter, script command, or mutation API is called. Two complete byte-identical traversals plus unchanged outer hit data are required. No pointer is retained after the synchronous call. A stable result still does not establish atomic impact timing.

Raw output includes call-local exact-instance identity, FormID, equip-slot mask, base/current condition, arithmetic condition ratio, raw armour rating, damage threshold, 32-bit biped flags, and armour flags. Equip slots do not establish anatomical coverage; traversal order does not establish layer order; zero observed worn armour does not establish bare skin. Accordingly `region_coverage_complete`, `layer_order_verified`, `impact_snapshot_verified`, `bare_region_verified`, `snapshot_authority`, and `armour_preview` all remain zero. Step 4A's ShadowAdapter/ArmourModel is not linked or called.

Capture is main-thread, exact-transaction, recursion-, taint-, session-, and epoch-guarded. It attempts at most 64 supported snapshots per load session and emits a final bounded summary. The double full-inventory scan is diagnostic only; a heavily loaded actor under automatic fire is the live stress checkpoint. An equipped-slot lookup or validated cache/invalidation design is required before always-on authority.

Offline evidence: 452 x86 `/W4 /WX` reader checks pass; final DLL is PE32 with exactly the two NVSE exports; new modules contain no patch or gameplay-write API; all 45 existing 3U1 patch-related source lines compare unchanged. The final compiled build is prepared but not installed. Damage replacement, armour wear, health, limb, condition, inventory, and equipment writes remain disabled.

# Historical packet: 3C5 / 310 guarded local-Z correction
